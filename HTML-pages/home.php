<!DOCTYPE HTML>
<html>
	<head>
	<title>Flower Store | Home</title>	
	</head>
	<body style="background-color: #17375d;">
		<br>
			<form action="" method="GET">
			<center>
			<div style="background-color: #ffffff; width: 850px;font-family: copperplate gothic light;">
					<div style="width: 840px;">
					<div style="float: left;margin: 20px 0 0 20px;">
							<a href="index.php"><img src="images/logo.png"></a>
					</div>
					
					<div style="width:80px; float: right; height: 25px; margin: 40px 35px 30px 0px; padding-top:5px; background-color: #cd15a6;border-radius: 5px;">
						<a href="" style="color: #ffffff; font-size: 0.8em; text-decoration: none;">Logout!</a>
					</div>
					<div style="width:80px; float: right; height: 25px; margin: 80px -80px 30px 0px; padding-top:5px; background-color: #cd15a6;border-radius: 5px;">
						<a href="settings.php" style="color: #ffffff; font-size: 0.8em; text-decoration: none;">Settings!</a>
					</div>
				</div>
				<hr size="2" color="#cd15a6" width="95%" style="clear: both;">
				<br>
				
				<table border="0" width="100">
					<tr>
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/1.jpg"><br><br>
							#1 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 500<br>
							<input type="checkbox" name="flower[]" value="1">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
						
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/2.jpg"><br><br>
							#2 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 600<br>
							<input type="checkbox" name="flower[]" value="2">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
						
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/3.jpg"><br><br>
							#3 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 700<br>
							
							<input type="checkbox" name="flower[]" value="3">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
						
						<td style="text-align: center;" >
							<img src="images/4.jpg"><br><br>
							
							#4 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 200<br>
							<input type="checkbox" name="flower[]" value="4">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
					</tr>
				</table>
				<br>			
				<hr size="1" color="c0c0c0" width="90%">
				<br>
				<table border="0" width="100">
					<tr>
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/5.jpg"><br><br>
							
							#5 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 100<br>
							<input type="checkbox" name="flower[]" value="5">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
						
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/6.jpg"><br><br>
							
							#6 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 100<br>
							<input type="checkbox" name="flower[]" value="6">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
						
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/7.jpg"><br><br>
							
							#7 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 1000<br>
							<input type="checkbox" name="flower[]" value="7">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
						
						<td style="text-align: center;" >
							<img src="images/8.jpg"><br><br>
							
							#8 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 750<br>
							<input type="checkbox" name="flower[]" value="8">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
						</td>
					</tr>
				</table>
				<br>			
				<hr size="1" color="c0c0c0" width="90%">
				<br>
				<table border="0" width="100">
					<tr>
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/9.jpg"><br><br>
							
							#9 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 870<br>
							<input type="checkbox" name="flower[]" value="9">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
							<span>
						</td>
						
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/10.jpg"><br><br>
							
							#10 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 1100<br>
							<input type="checkbox" name="flower[]" value="10">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
							<span>
						</td>
						
						<td style="text-align: center;border-right: 1pt solid #cccccc;" >
							<img src="images/11.jpg"><br><br>
							
							#11 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 250<br>
							<input type="checkbox" name="flower[]" value="11">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
							<span>
						</td>
						
						<td style="text-align: center;" >
							<img src="images/12.jpg"><br><br>
							
							#12 &nbsp;&nbsp;&nbsp;&nbsp;Rs. 320<br>
							<input type="checkbox" name="flower[]" value="12">
							<input type="submit" name="submit" value = "Add!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 50px; border-radius:5px; ">
							<span>
						</td>
					</tr>
				</table>
				<br><br>
				
				<input type="submit" name="buynow" value = "Buy Now!" style=" background:#cd15a6;border:none; color:white; font-family:georgia; font-size:14px; height: 25px; width: 100px; border-radius:5px; ">
				<br><br>
				<hr size="1" color="c0c0c0" width="95%">
				<br>
			</div>
			</center>
		</form>
		<br>
	</body>
</html>